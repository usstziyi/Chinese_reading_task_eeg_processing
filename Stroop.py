...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\Psychopy\\Stroop.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    color_stim = visual.TextStim(win=win, name='color_stim',
        ...
        languageStyle='LTR',
        depth=0.0);
    color_resp = keyboard.Keyboard(deviceName='defaultKeyboard', backend='PsychToolbox')
    ...
    while continueRoutine:
        ...
        if key_resp.status == STARTED and not waitOnFlip:
            theseKeys = key_resp.getKeys(keyList=['up'], ignoreKeys=["escape"], waitRelease=False)
    ...
    for thisTrials_loop in trials_loop:
        ...
        trial = data.Routine(
            name='trial',
            components=[color_stim, color_resp, fixation],
        )
        ...
        # create starting attributes for color_resp
        color_resp.keys = []
        color_resp.rt = []
        _color_resp_allKeys = []
        ...
        while continueRoutine and routineTimer.getTime() < 2.5:
            ...
            if color_stim.status == STARTED:
                ...
                if tThisFlipGlobal > color_stim.tStartRefresh + 2.0-frameTolerance:
            ...
            # if color_resp is starting this frame...
            if color_resp.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                # keep track of start time/frame for later
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'color_resp.started')
                # update status
            ...
            # if color_resp is stopping this frame...
            if color_resp.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > color_resp.tStartRefresh + 2.0-frameTolerance:
                    # keep track of stop time/frame for later
                    color_resp.tStop = t  # not accounting for scr refresh
                    color_resp.tStopRefresh = tThisFlipGlobal  # on global time
                    color_resp.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'color_resp.stopped')
                    # update status
                    color_resp.status = FINISHED
                    color_resp.status = FINISHED
            if color_resp.status == STARTED and not waitOnFlip:
                theseKeys = color_resp.getKeys(keyList=['left','right','down'], ignoreKeys=["escape"], waitRelease=False)
                ...
                if len(_color_resp_allKeys):
                    ...
                    color_resp.duration = _color_resp_allKeys[-1].duration
                    # was this correct?
                    if (color_resp.keys == str(corr_Ans)) or (color_resp.keys == corr_Ans):
                        color_resp.corr = 1
                    else:
                        color_resp.corr = 0
                    # a response ends the routine
        ...
        trials_loop.addData('color_resp.keys',color_resp.keys)
        trials_loop.addData('color_resp.corr', color_resp.corr)
        if color_resp.keys != None:  # we had a response
            trials_loop.addData('color_resp.rt', color_resp.rt)
            trials_loop.addData('color_resp.duration', color_resp.duration)
    ...
    last = data.Routine(
        name='last',
        components=[final_text, key_resp_2],
    )
    ...
    while continueRoutine and routineTimer.getTime() < 5.0:
        ...
        if final_text.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > final_text.tStartRefresh + 5.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        # has the Routine been forcibly ended?
        if last.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in last.components:
